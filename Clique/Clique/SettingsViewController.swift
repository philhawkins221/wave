    ...
    @IBAction func spotifyChanged(sender: AnyObject) {
        if spotifyswitch.on {
            ...
            if (spotifysession == nil) {
                ...
                if UIApplication.sharedApplication().openURL(loginURL) {
                    currentclique.spotify = true
                    
                    var player: SPTAudioStreamingController?
                 
                    player = SPTAudioStreamingController(clientId: kClientId)
                    player?.loginWithSession(spotifysession, callback: { (error) -> Void in
                        if (error != nil) {
                            print("*** Enabling playback got error: \(error)")
                            return
                        } else {
                            
                            SPTRequest.requestItemAtURI(NSURL(string: "spotify:album:3vGtqTr5he9uQfusQWJ0oC"), withSession: spotifysession, callback: { (error, album) -> Void in
                                if (error != nil) {
                                    print("*** Album lookup got error \(error)")
                                    return
                                }
                                player?.playTrackProvider(album as! SPTAlbum, callback: nil)
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        for i in 0 ..< 3 {
            if i == indexPath.row {
                ...
                if i == 0 { emptydirective = .nothing }
                else if i == 1 { emptydirective = .library }
                else if i == 2 { emptydirective = .magic }
        ...
        if indexPath.row == 0 { //prepare for nothing
            return
        } else if indexPath.row == 1 { //prepare for library
                ...
                case .Success:
                    if let value = response.result.value {
                        ...
                        for song in json["library"].arrayValue {
                            ...
                            dispatch_async(dispatch_get_main_queue(), { [weak pm = PlayerManager.sharedInstance()] in
                                pm!.library.append((newsong, false))
                            })
                        }
                    }
        ...
        } else if indexPath.row == 2 { //prepare for magic
            return
        }
    }
